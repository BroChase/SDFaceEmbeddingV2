...
class Handler:
    ...
    def on_draw(self, wid, cr):
        ...
        if THREAD is None or not THREAD.isAlive():
            THREAD = threading.Thread(target=CAM.get_masked_img(win_w))
            THREAD.start()

        ims,img_size = CAM.masked_img_buffer, CAM.image_size
...
if __name__ == "__main__":
    ...
    win_w = window.get_size()[0]
    CAM = Camera(10,int(win_w))
